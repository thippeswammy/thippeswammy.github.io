...
JS_DATA_FILE = "../web/projects_data.js" # Path to projects_data.js
...
# Cluster Mapping (GitHub Topics -> Cluster ID)
CLUSTER_MAP = {
...
def extract_highlights(repo_name):
    """Tries to find bullet points in README.md."""
        ...
        if os.path.exists(path):
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    content = f.read()
                    # Find bullet points after headers like Features, Highlights, etc.
                    matches = re.findall(r'(?m)^[*-]\s+(.+)', content)
                    if matches:
                        highlights = [m.strip() for m in matches[:4]] # Take first 4
                        break
            except: pass
    return highlights

def generate_portfolio_js(meta_repos, log_callback):
    """Generates the projects_data.js file from meta info."""
    log_callback("Building portfolio data...")
    projects = []
    
    for name, data in meta_repos.items():
        # Get extra info from meta if saved, else use defaults
        info = data.get("info", {})
        if not info: continue # Skip if we don't have basic info yet
        ...
        # Extract highlights from local file
        highlights = extract_highlights(name)
        if not highlights:
            highlights = [f"Language: {info.get('language', 'N/A')}", f"Last updated on GitHub"]
    ...
    try:
        ...
        with open(JS_DATA_FILE, "w", encoding="utf-8") as f:
            f.write(js_content)
        log_callback(f"SUCCESS: {len(projects)} projects exported to {os.path.basename(JS_DATA_FILE)}")
        return True
    except Exception as e:
        log_callback(f"FAILED to write JS: {e}")
        return False
...
class RepoSyncGUI:
    ...
    def setup_ui(self):
        ...
        self.sidebar = ttk.Frame(main, padding=20, borderwidth=1, relief="flat")
        ...
        self.build_btn = tk.Button(self.sidebar, text="BUILD PORTFOLIO DATA", command=self.build_portfolio, 
                                  bg=COLOR_PRIVATE, fg="white", font=("Segoe UI", 9, "bold"), relief="flat", pady=8)
        self.build_btn.pack(fill="x", pady=10)
        ttk.Label(self.sidebar, text="Generates projects_data.js from\nsynced READMEs and metadata.", 
                  font=("Segoe UI", 7), foreground=COLOR_FORK, justify="left").pack(anchor="w")

        # Repo List
        ttk.Label(self.content, text="REPOSITORIES", font=("Segoe UI", 12, "bold")).pack(anchor="w", pady=(0, 10))
        list_f = ttk.Frame(self.content)
        list_f.pack(fill="both", expand=True)
        self.canvas = tk.Canvas(list_f, bg="white", highlightthickness=0)
        self.scrollbar = ttk.Scrollbar(list_f, orient="vertical", command=self.canvas.yview)
    ...
    def on_resize(self, e): self.canvas.itemconfig(self.canvas_win, width=self.canvas.winfo_width())
    ...
    def build_portfolio(self):
        if not self.meta.get("repos"):
            messagebox.showwarning("No Data", "Please sync at least one repository first.")
            return
    ...
    def start_sync(self):
        ...
        self.meta["selected_repos"] = sel_names
        save_meta(self.meta)
    ...
    def run_engine(self, repos):
        total = len(repos); updated = 0
        for i, r in enumerate(repos):
            try:
                # Enrich meta with GitHub info for JS generator
                if r['name'] not in self.meta["repos"]: self.meta["repos"][r['name']] = {"files": {}}
                self.meta["repos"][r['name']]["info"] = {
                ...
                count = download_md_files(self.username, r, self.meta, self.log)
                updated += count
            except Exception as e: self.log(f"FAILED {r['name']}: {e}")
            self.prog_var.set(((i + 1) / total) * 100)
        self.meta["last_sync"] = datetime.now().strftime("%Y-%m-%d %H:%M")
        save_meta(self.meta)
